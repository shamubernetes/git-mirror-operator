...
import (
	...
	githubwebhook "github.com/shamubernetes/git-mirror-operator/internal/webhook"
...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "git-mirror-operator.shamubernetes.com",
	...
	if err := (&controller.GitMirrorReconciler{
		Client:           mgr.GetClient(),
		Scheme:           mgr.GetScheme(),
		DefaultSyncImage: syncImage,
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "GitMirror")
		os.Exit(1)
	}
	// +kubebuilder:scaffold:builder

	githubWebhookServer := githubwebhook.NewServerWithReaders(mgr.GetClient(), mgr.GetAPIReader(), syncImage, mgr.GetScheme())
	mux := http.NewServeMux()
	mux.HandleFunc("/webhooks/github", githubWebhookServer.HandleGitHub)
	mux.HandleFunc("/healthz", func(w http.ResponseWriter, _ *http.Request) { w.WriteHeader(http.StatusOK) })
	mux.HandleFunc("/readyz", func(w http.ResponseWriter, _ *http.Request) { w.WriteHeader(http.StatusOK) })
	if err := mgr.Add(&httpServer{addr: githubWebhookAddr, handler: mux}); err != nil {
		setupLog.Error(err, "unable to add GitHub webhook server")
		os.Exit(1)
	}
