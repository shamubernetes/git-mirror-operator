...
# ghcr.io/shamubernetes/git-mirror-operator-bundle:$VERSION and ghcr.io/shamubernetes/git-mirror-operator-catalog:$VERSION.
IMAGE_TAG_BASE ?= ghcr.io/shamubernetes/git-mirror-operator
...
SYNC_IMG ?= git-mirror-sync:latest
...
check-generated-drift: ## Verify generated files and module metadata are current.
	git diff --exit-code -- go.mod go.sum api config
	@test -z "$$(git status --porcelain --untracked-files=all -- api config)" || { \
		git status --short --untracked-files=all -- api config; \
		exit 1; \
	}
...
.PHONY: build-sync
build-sync: fmt vet ## Build sync runner binary.
	go build -o bin/git-mirror-sync ./sync
...
docker-build-sync: ## Build docker image with the sync runner.
	$(CONTAINER_TOOL) build -f Dockerfile.sync -t ${SYNC_IMG} .
...
if [ ! -f "$(1)" ] && [ -n "$(GOENV_GOBIN)" ] && [ -f "$(GOENV_GOBIN)/$(notdir $(1))" ]; then cp "$(GOENV_GOBIN)/$(notdir $(1))" "$(1)" ; fi ;\
mv $(1) $(1)-$(3) ;\
} ;\
ln -sf $(1)-$(3) $(1)
endef
